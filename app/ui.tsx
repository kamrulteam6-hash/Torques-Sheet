...
/**
 * The trouble-code guide has grown into hundreds of pages across dozens of
 * vehicles, so the homepage can no longer list all of it — that reads as one
 * make dominating the page rather than a teaser. This samples one code per
 * distinct vehicle (falling back to the code itself when a guide predates
 * the `vehicle` field) so the homepage preview stays a genuine cross-section
 * instead of however many codes the first vehicle in the array happens to have.
 */
function sampleTroubleCodes(limit: number) {
  const seen = new Set<string>();
  const sample: typeof troubleCodeGuides = [];
  for (const guide of troubleCodeGuides) {
    // Guides written before the `vehicle` field existed are all the same
    // truck (F-150) — group them under one key rather than letting their
    // distinct codes slip past the per-vehicle dedup below.
    const key = guide.vehicle?.about ?? "legacy-f150";
...
/** One tool per family, so the homepage teaser spans the section rather than clustering in one corner of it. */
...
function featuredTools() {
  return FEATURED_TOOL_SLUGS.map((slug) => tools.find((t) => t.slug === slug)).filter((t): t is NonNullable<typeof t> => Boolean(t));
}
